...
def open_doc_link():
    webbrowser.open_new("https://github.com/tltltltltltltltl/courier-delivery-solver#readme")


def open_github_link():
    webbrowser.open_new("https://github.com/tltltltltltltltl/courier-delivery-solver")
...
def open_qq_link():
    webbrowser.open_new("https://github.com/tltltltltltltltl/courier-delivery-solver/issues")
...
def show_algorithm_parameters(algo_name):
    ...
    required_parameters, value_type, default_value = get_required_parameters(
        path=ROOT_DIR / "llm4ad" / "method" / algo_name / "paras.yaml")
...
def show_problem_parameters(problem_name):
    ...
    if problem_name[-8:] == 'co_bench':
        yaml_file_path = ROOT_DIR / "llm4ad" / "task" / objectives_var.get() / "co_bench" / problem_name / "paras.yaml"
    else:
        yaml_file_path = ROOT_DIR / "llm4ad" / "task" / objectives_var.get() / problem_name / "paras.yaml"
...
def problem_type_select(event=None):
    ...
    path = ROOT_DIR / 'llm4ad' / 'task' / objectives_var.get()
    ...
    if objectives_var.get() == 'optimization':
        path = ROOT_DIR / 'llm4ad' / 'task' / objectives_var.get() / 'co_bench'
        if path.exists():
            for name in os.listdir(path):
                full_path = os.path.join(path, name)
    ...
    if default_problem_index is None and problem_listbox.size() > 0:
...
if __name__ == '__main__':
    ...
    try:
        root.iconbitmap(str(GUI_DIR / 'image' / 'icon.ico'))
    except Exception:
        pass
    ...
    problem_param_frame = ttk.Labelframe(container_frame_2, text="courier_bundle_assignment", bootstyle="warning")
    ...
    radiobutton_list = []
    for _, dict_name, _ in os.walk(ROOT_DIR / 'llm4ad' / 'task'):
